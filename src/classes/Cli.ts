...
class Cli {
  ...
  createTruck(): void {
    inquirer
      ...
      .then((answers: {color: string, make: string, model: string, year: number, weight: number, topSpeed: number, towingCapacity: number}) => {
...
if (vehicleToTow === selectedTruck) {
  console.log('The truck cannot tow itself. Please select another vehicles.');
  this.performActions();
} else {
      ...
      .then((answers) => {
        ...
        if (answers.action === 'Print details') {
          ...
          for (let i = 0; i < this.vehicles.length; i++) {
            ...
            console.log(`${selectedVehicle.make} ${selectedVehicle.model} is doing a wheelie!`);
        }
